...
class Pager
{
	 ...
	 * @return void
	 */
	public function setRequest($page_name = 'p', $query = [])
	{
		...
		if ($this->_url == null) {
            ...
            if (!empty($query)) {
                ...
                $pager_query = (!empty($query)) ? '&'.http_build_query($query) : '';
            }
			$url = $path . '/?'.$page_name.'=%page%'.$pager_query;
			$this->setUrl($url);
		}
		$page = (isset($_GET[$page_name])) ? $_GET[$page_name] : null;
		if (is_numeric($page)) {
	 ...
	 * @return void
	 */	
	public function outputTag($render = true)
	{
		$tag = $this->getTag();
        if ($render === false) return $tag;
		echo $tag;
	}
